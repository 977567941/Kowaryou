...
class Spider(Spider):
    def localProxy(self, params):
        try:
            if params.get('type') == 'artjx':
                data =  self.art_jx(params)
            else:
                data = '请按要求传参'
    ...
    def art_jx(self, params):
        v = params.get('v')
        if not v:  return '链接为空'
        timeout = params.get('timeout') or 10
        api = params.get('api')
        if not v:  return 'api为空'
        key = params.get('key') or 'ARTPLAYER2023217'
        iv = params.get('iv') or 'Artplayerapiban1'
        ...
        try:
            data = self.post(api, data=body, headers=headers, timeout=timeout, verify=False, allow_redirects=True).json()
        except Exception:
            return '请求或解析失败'
