...
class Spider(Spider):
    ...
    def szy_jx(self, params):
        ...
        host = params.get('domain') or 'https://ppl.sszzyy.com'
        ...
        try:
            data = self.post(f"{host}/api.php", data=body, headers=headers, timeout=timeout, verify=False, allow_redirects=True).json()
        except Exception:
            return '请求或解析失败'
