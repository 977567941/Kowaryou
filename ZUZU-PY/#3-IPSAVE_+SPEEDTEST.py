...
with open("合并IPkkk.txt", "w", encoding="utf-8") as output:
...
file_paths = ["天津联通.txt", "山西联通.txt","安徽电信.txt", "河南联通.txt", "河南电信.txt", "福建电信.txt", "江苏电信.txt", "贵州电信.txt", "四川联通.txt", "四川电信.txt", "重庆联通.txt", "重庆电信.txt","山东电信.txt","广东电信.txt","广西电信.txt","江西电信.txt","河北电信.txt","浙江电信.txt","湖北电信.txt","湖南电信.txt","辽宁联通.txt","陕西电信.txt",'JIEXI-NG.txt']  #替换为实际的文件路径列表
...
with open('合并IPkkk.txt', 'r', encoding='utf-8') as file, open('IP_save.txt', 'a', encoding='utf-8') as IP_save:
...
# 打开文件，并设置编码为utf-8
with open('合并IP.txt', 'r', encoding='utf-8') as file:
    for line in file:
        # 检查行中是否包含1个逗号
        if line.count(',') == 1:
            # 将整行定义为一个channel
            ...
            if frame_count > 190:  # <限定合格IP判定的帧数
                tested_ips[ip_part] = 'ok'
            else:
                # 如果没有读取到任何帧，则标记为已测试
                tested_ips[ip_part] = 'tested'

            # 释放VideoCapture对象
            cap.release()

# 测试结束后，将包含ok的IP的channel写入新文件
with open('合并SPEEDTEST.txt', 'w', encoding='utf-8') as file:
    with open('合并IP.txt', 'r', encoding='utf-8') as input_file:
        for line in input_file:
            ...
            if line.count(',') == 1:
                ...
                if rtp_pos != -1:
                    ...
                    # 检查这个IP是否在测试成功的IP字典中
                    if channel_ip in tested_ips and tested_ips[channel_ip] == 'ok':
                        # 如果在，就写入这个channel到输出文件
                        file.write(line)
...
file_paths = ["合并SPEEDTEST.txt","JIEXI-OK.txt"]  #把测速结果和JIEXI合并到一起
...
##os.remove("合并IPkkk.txt")

os.remove("合并SPEEDTEST.txt")
